  ...
  "lessons": [
    ...
    },
    {
      ...
      "concept": {
        ...
        "keyPoints": [
          "现代 AMD GPU 有 3 个关键 BAR：BAR0（VRAM aperture）、BAR2（Doorbell）、BAR5（MMIO 寄存器）",
      ...
      "debugExercise": {
        ...
        "solution": "正确做法：使用 ioremap() 将物理地址映射到虚拟地址，然后用 readl() 读取：`void __iomem *bar5 = ioremap(pci_resource_start(pdev, 5), pci_resource_len(pdev, 5)); uint32_t val = readl(bar5); iounmap(bar5);`。直接解引用物理地址会导致页错误（Page Fault），因为内核的页表中没有该物理地址的映射。"
      },
      "interviewQuestion": {
