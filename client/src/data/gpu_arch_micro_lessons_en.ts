...
int main()
{
    ...
    printf("OK: %d elements, grid=%d block=%d\\n",
          ...
          miniLab: {
            ...
            steps: [
              'With a ROCm environment: save the program above, hipcc vecadd_full.hip.cpp -o vecadd && ./vecadd, confirm it prints OK; without an environment do the compile-time parts on godbolt (steps 3 and 4 as written)',
	...
	/* ... then write the fence address and sequence number ... */
          ...
          miniLab: {
            ...
            steps: [
              ...
              'With a ROCm machine, run rocminfo | grep -iA2 cache to cross-check the measurements',
        ...
        // ── Lesson 1.5.3.4 ────────────────────────────────────
        {
          ...
          concept: {
            ...
            explanation: [
              ...
              "The strategic meaning of all this is user-mode queues: after creating a queue, a process holds a mapping of its own ring and its own doorbell page; submitting work = writing its own ring + ringing its own doorbell — never entering the kernel! The ioctl happens exactly once, at queue creation. ROCm/KFD has worked this way for years, and the graphics stack is moving to the same model. The price is a more complex debugging and security story (the kernel no longer sees each submission) — which is why MES firmware quality directly determines the bring-up experience of new hardware, and MES-related errors in dmesg are a high-frequency keyword when troubleshooting GFX11+.",
            ],
            keyPoints: [
              ...
              'User-mode queue = own ring + own doorbell, zero-ioctl submission — long-standing reality in KFD/ROCm, the direction for graphics.',
