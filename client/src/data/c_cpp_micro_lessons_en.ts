...
/* Each .c compiles fine, but:
   ...
   /usr/bin/ld: multiple definition of 'g_debug_level' */`,
        ...
        },
        {
          ...
          concept: {
            summary: 'The sizes of C\'s basic integer types are implementation-defined; narrow integers are automatically promoted to int in expressions; drivers use fixed-width types (u8/u16/u32/u64) to guarantee the exact widths hardware registers require.',
            explanation: [
              ...
              'Integer promotion: any integer narrower than int (char, short, bitfields, u8, u16) is promoted to int before arithmetic. So with u8 a=200, b=100; a+b is actually computed as 300 in int and does not wrap — frequently surprising.',
              'Usual arithmetic conversions: when signed and unsigned mix, the signed operand is usually converted to unsigned. Classic trap: unsigned a=0; a-1 is not -1 but 0xFFFFFFFF; if (a - b < 0) is always false when a and b are unsigned.',
            ...
            keyPoints: [
              ...
              'Integers narrower than int are promoted to int before arithmetic',
              'signed ⊕ unsigned mixing → usually converts to unsigned; a frequent "negative becomes huge positive" bug',
            ...
            buggyCode: `static irqreturn_t my_gpu_irq(int irq, void *arg)
{
	...
	return IRQ_HANDLED;
          ...
          interviewQ: {
            ...
            amdContext: "Concurrency-context questions are a fixed segment of AMD kernel-team phone screens, most commonly \"find the bugs in this interrupt handler\" — the four error classes in this lesson's debug exercise cover 90% of what gets asked, and volunteering amdgpu's irq→work separation design is a significant plus.",
          },
        },
      ],
    },
