  ...
  "lessons": [
    ...
    },
    {
      ...
      "concept": {
        ...
        "keyPoints": [
          "GPUs typically have 3 BARs: BAR0 (VRAM), BAR2 (registers), BAR4 (Doorbell)",
      ...
      "debugExercise": {
        ...
        "solution": "Correct approach: use ioremap() to map the physical address to a virtual address, then use readl() to read: `void __iomem *bar2 = ioremap(pci_resource_start(pdev, 2), pci_resource_len(pdev, 2)); uint32_t val = readl(bar2); iounmap(bar2);`. Directly dereferencing a physical address causes a page fault because the kernel's page table has no mapping for that physical address."
      },
      "interviewQuestion": {
