...
int main()
{
    ...
    printf("OK: %d elements, grid=%d block=%d\\n",
          ...
          miniLab: {
            ...
            steps: [
              '有受支持的 ROCm 环境：保存上面的程序，hipcc vecadd_full.hip.cpp -o vecadd && ./vecadd，确认输出 OK（注意 RX 7600 XT/gfx1102 不在 ROCm 官方支持列表，见环境搭建页的兼容性说明）；没有环境就在 godbolt 上做编译期部分（第 3、4 步照做）',
	...
	/* ... 随后写 fence 地址与序号 ... */
          ...
          miniLab: {
            ...
            steps: [
              ...
              '有受支持的 ROCm 机器可运行 rocminfo | grep -iA2 cache 对照实测（RX 7600 XT 不在官方支持列表，本步可跳过或查表完成）',
        ...
        // ── Lesson 1.5.3.4 ────────────────────────────────────
        {
          ...
          concept: {
            ...
            explanation: [
              ...
              '这套机制的战略意义是用户态队列（user-mode queues）：进程创建队列后，拿到自己 ring 的映射和自己的 doorbell 页，提交工作 = 写自己的 ring + 敲自己的 doorbell，全程不进内核！ioctl 只在建队列时发生一次。ROCm/KFD 多年来就这么工作；图形侧的 amdgpu 用户态队列已随 Linux 6.16 合入实验性支持（GFX11/GFX12，内核文档见 docs.kernel.org/gpu/amdgpu/userq.html），Mesa 25.2 起支持图形用户队列——默认路径目前仍是内核队列。代价是调试与安全模型更复杂（内核看不到每次提交了）——这也是为什么 MES 固件的质量直接决定新硬件的启动体验，dmesg 里 MES 相关报错是 GFX11+ 排障的高频关键词。',
            ],
            keyPoints: [
              ...
              '用户态队列 = 自己的 ring + 自己的 doorbell，提交零 ioctl——KFD/ROCm 的既有现实；图形侧已于 Linux 6.16 合入实验性支持（GFX11+）。',
