...
• Deep experience with ROCm/HIP performance optimization
  on data-center GPUs`,
        ...
        // ── Lesson 11.3.3 ────────────────────────────────────
        {
          id: '11-3-3',
          number: '11.3.3',
          title: 'intern/new-grad 通道：一场完全不同的面试',
          titleEn: 'The Intern/New-Grad Track: A Different Interview Entirely',
          duration: 20,
          tags: ['intern', 'new-grad', 'referral', 'interview-strategy'],
          concept: {
            summary:
              '本模块前面的内容都以正职岗叙事——那里 4-8 年经验墙真实存在。但 intern 和 new-grad 通道的游戏规则完全不同：没有年限筛选、面试考基础而非履历、领域知识从"要求"变成"罕见的加分项"。如果你有内推，唯一剩下的变量就是面试表现本身——而它的构成与正职面试有系统性差异，备考策略必须换。',
            explanation: [
              '权重结构对比。正职驱动岗面试：过往项目深挖（约四成）+ 领域系统设计（三成）+ 内核/并发专题（两成）+ 编码（一成）。intern/new-grad 面试：基础编码题（约 35%）+ C/C++ 与 OS 基础（30%）+ 项目讲述（20%）+ 领域知识（15%，且是加分性质）。注意两个反转：编码题从最不重要变成最重要；领域知识从硬门槛变成差异化武器。用正职的备考策略打 intern 面试，会赢下 15% 输掉 35%。',
              '这对本站学习者意味着什么：你在领域知识上会碾压同池候选人（绝大多数 intern 候选人说不出 wavefront 和 warp 的区别，更不用说 ring buffer 和 doorbell），但如果三个月没写过算法题，会倒在第一轮 coding screen 上。诚实的自我评估：本站没有覆盖 DS/algo——这块必须外部补（每天 1-2 题，数组/字符串/链表/哈希/树为主，驱动方向的 intern 面试极少考 hard 动态规划）。练习页的"实习/校招特训"过滤器把站内题库收窄到 intern 真实考察范围，配合外部刷题使用。',
              '内推的正确用法：内推解决的是"简历被人看到"，不是"面试放水"。拿到内推后要主动做三件事：①问清楚目标组（display/kernel/tools/ROCm 的面试重点完全不同——display 组会问 KMS/atomic，ROCm 组会问 HIP/内存模型）；②问清楚时间窗口（intern 岗季节性强，北美夏季岗常提前 6-9 个月开放，窗口开了就投，不等"准备完美"）；③给内推人一份 30 秒可转述的自我介绍（"他自学 AMD 驱动栈一年，做了个双语学习平台，给 amd-gfx 发过补丁"——让内推人替你说出口的话，先替他写好）。',
              '你的差异化武器要主动出鞘：这个学习平台本身就是双重作品——全栈工程项目（React/TS/双语架构/内容不变量测试/CI）+ 领域承诺证明。面试的"介绍一个项目"环节永远选它：准备一个 2 分钟版本（是什么、为什么做、两个最难的技术决策）和一个 30 秒电梯版本。再叠加任何一条真实上游痕迹（lab-8 的 issue triage 评论、lab-7 的 lore 链接），你就是那个面试官会记住的候选人。最后的心态校准：intern 面试的通过率天然高于正职，且 return offer 是进入正职概率最高的路径——把目标定为"拿到 intern，然后在 intern 期间用真实工作转正"，比直接冲正职的期望值高得多。',
            ],
            keyPoints: [
              '权重反转：intern 面试 coding 约 35% + 基础 30%，领域知识只占 15% 但是罕见加分项。',
              '本站盲区自知：DS/algo 必须外部补（每天 1-2 题 easy/medium）；站内用"实习/校招特训"过滤器收窄题库。',
              '内推三问：目标组别、时间窗口、给内推人的 30 秒转述稿。',
              '学习平台本身是最强项目素材：准备 2 分钟版与 30 秒版讲稿。',
              'intern → return offer 是进正职概率最高的路径；窗口开了就投，不等完美。',
            ],
          },
          diagram: {
            title: '正职 vs intern 面试的权重反转',
            content: `正职驱动岗                  intern/new-grad
项目深挖  ████████ ~40%     编码题     ███████ ~35%
领域设计  ██████ ~30%       C/OS 基础  ██████ ~30%
内核专题  ████ ~20%         项目讲述   ████ ~20%
编码      ██ ~10%           领域知识   ███ ~15% (加分)

你的处境: 领域知识超配 | 编码手感 = 唯一短板
策略: 每天 1-2 题恢复手感, 让领域优势在
      "项目讲述"与加分环节最大化输出`,
            caption: '同一家公司、同一个组，两条通道考的是两场不同的试。备考资源要按右边的权重分配。比例为经验性示意，随组别浮动。',
          },
          codeWalk: {
            title: '给内推人的转述稿 + 简历项目段（可直接改写）',
            language: 'text',
            file: '内推沟通模板（按真实情况裁剪）',
            code: `── 给内推人的 30 秒转述稿 ──────────────────
"他是 [学校/专业] 的应届生，过去一年系统自学了
AMD Linux 驱动栈——自己搭了一个双语学习平台
（14 个模块、9 个实验，内容对照 ROCm/ISA 官方文
档核实），在 RX 7600 XT 上做过 GPU hang 分析和
ftrace 追踪，[给 amd-gfx 发过补丁 / 在 drm/amd
tracker 上做过 issue triage]。想找 [组名] 的
intern 机会，链接在这：[网站] [lore/GitLab]"
...
- Labs produce public artifacts: kernel builds,
  ...
  [+ upstream patch on lore / issue triage link]`,
            explanation:
              '两段文字的共同纪律：每个名词都可点击验证、没有一个形容词需要面试官信任你的自评（对照 11.3.2 的红线清单自查）。给内推人的版本刻意口语化——他要在走廊里 30 秒说完；简历版每条 bullet 都以动词开头、以链接收尾。方括号内容按你的真实进度增删，宁可短不可虚。',
          },
          miniLab: {
            title: '把内推变成面试：三件套准备',
            objective: '在 48 小时内产出内推所需的全部材料，并完成第一次投递前检查。',
            steps: [
              '写出给内推人的 30 秒转述稿（照 codeWalk 模板改写成你的真实情况），发给内推人并直接问两个问题：哪个组在招 intern？什么时候开放投递？',
              '用 11.3.2 的公式 + 本课模板改好简历 Projects 段，找一个英语好的人读 60 秒——他能复述出你做了什么，才算合格',
              '准备网站的 2 分钟讲稿：是什么（30 秒）→ 为什么做（30 秒）→ 两个最难的技术决策（60 秒，例如"内容不变量测试怎么防止事实回归"）——录音自听一遍',
              '开始每日 coding 训练：今天做 2 道 easy（数组/字符串），记录用时；本周完成 10 道，并用练习页"实习/校招特训"过滤器过一轮站内基础题',
              '建立投递跟踪表：岗位链接、投递日期、内推确认、跟进日期——第一行填上内推人给你的岗位',
            ],
            expectedOutput:
              '48 小时后你应有：内推人手里的转述稿 + 目标组和时间窗的答案、可发送的简历、2 分钟项目讲稿录音、coding 训练记录第一页、投递跟踪表第一行。这五样合起来 = 内推的全部价值被兑现。',
            hint: '第 1 步最重要且最容易拖延——转述稿今天就发出去；后面四步都由它给出的 deadline 驱动。',
          },
          debugExercise: {
            title: '这场 intern 面试哪里打错了牌？',
            language: 'text',
            question: '一位学完本站的候选人在 AMD intern 面试中的三个片段。逐个指出问题并给出正确打法。',
            buggyCode: `片段 A（coding 环节，反转链表题）:
"这题其实和内核的 list_head 很像，我先讲讲侵入式
链表的设计哲学……container_of 的原理是……"
（8 分钟后还没写第一行代码）

片段 B（被问"介绍一个项目"）:
"我做过一个学习网站，用 React 写的，就是把学的
东西整理了一下，没什么特别的。主要还是想聊聊我
对 amdgpu 驱动的理解，比如 TTM 的驱逐机制……"

片段 C（行为面试，"遇到过最大的困难？"）:
"学 GPUVM 页表的时候特别难，看了三遍才懂。
后来看懂了。"`,
            hint: 'A：面试官此刻在评估什么能力？B：候选人把自己最强的证据说成了什么？C：STAR 结构缺了哪几个字母？',
            answer:
              'A（用领域知识逃避考察目标）：coding 环节评估的是"能不能把简单问题正确写出来"——扯侵入式链表 8 分钟不动笔，面试官记录的是"无法进入执行状态"。正确打法：30 秒内开始写，先给朴素解再优化；领域联系可以在写完后用一句话点缀（"顺带一提内核的 list_head 是这个思路的侵入式变体"）——那才是加分，前置就是逃避。B（自贬最强资产 + 抢答错题）：把双语平台说成"整理了一下"，是把面试里唯一差异化的证据主动降级；而"想聊 TTM"是在替面试官出题——intern 面试官问项目是想看工程判断与表达，不是驱动知识竞赛。正确打法：按 2 分钟讲稿走完（数字 + 两个技术决策 + 链接），把 TTM 留给面试官主动追问。C（STAR 只剩 S）：没有 Task（当时要达成什么）、没有 Action（具体做了什么——画了页表层级图？写了测试模块验证理解？）、没有 Result（产出在哪）。空洞的"看了三遍就懂了"传递的信息量为零。三个片段的共同病根：没有意识到 intern 面试是一场"按对方的评分表输出"的考试——你的领域深度只有放进对方的评分格子里才计分。',
          },
          interviewQ: {
            question: '（行为题示范）你没有任何实习经历，为什么我们应该给你这个 GPU 驱动组的 intern 机会？',
            difficulty: 'medium',
            hint: '不辩解"没有经历"，直接用证据链重定义问题：你有的恰恰是大多数 intern 候选人没有的东西。',
            answer:
              '示范回答框架（按真实情况填充）："我没有正式实习经历，但我有一年的可验证自学轨迹，而且它恰好就是这个组的日常技术栈。三个证据：第一，我建了一个双语的 AMD 驱动学习平台——不是笔记堆，是有 30 项内容事实测试、对照 ROCm 和 ISA 官方文档核实的工程项目，这是链接。第二，我在自己的 RX 7600 XT 上做过完整的调试实践：GPU hang 的 dmesg/devcoredump 分析、ftrace 追 fence、KUnit 测试报告，都在这个 portfolio 仓库里。第三，我走通过真实的上游流程——[给 amd-gfx 发过 kernel-doc 修复/在 drm/amd tracker 上 triage 过 issue]，lore/GitLab 链接可以现在打开。我清楚 intern 的日常是给定任务的执行与学习，这三样证明的正是：不需要有人教我怎么搭环境、读代码、走流程——我第一天就能开始干活。"要点：①每个主张跟一个链接；②结尾落在"降低你带我的成本"——这是 intern 招聘真正的决策函数；③语气是陈述事实而非恳求机会。',
            amdContext: '这是 intern 面试的必考题型（形式可能是"为什么选你/为什么选我们组"）。有内推时面试官大概率已经瞄过你的链接——答案与材料相互印证时，可信度倍增；相互矛盾时，一票否决。',
          },
        },
      ],
    },
