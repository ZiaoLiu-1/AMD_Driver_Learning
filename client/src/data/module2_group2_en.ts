  ...
  "lessons": [
    ...
    },
    {
      ...
      "debugExercise": {
        ...
        "hint": "When VRAM is exhausted, the driver evicts some BOs to GTT (system memory). GPU access to those objects now travels over PCIe, dropping from local-VRAM bandwidth to PCIe-limited bandwidth.",
        "solution": "Diagnostic steps: 1) `cat /sys/class/drm/card0/device/mem_info_vram_used` — check whether usage is approaching 16384 MB on an RX 7600 XT; 2) `sudo cat /sys/kernel/debug/dri/0/amdgpu_eviction_stats` — count eviction events; 3) use `radeontop` to watch VRAM and GTT usage in real time. If VRAM is full and GTT usage spikes simultaneously, large-scale eviction is occurring. Fix: lower the game's texture quality settings, or upgrade to a GPU with more VRAM."
      },
      "interviewQuestion": {
        ...
        "answer": "VRAM is GPU-local memory (GDDR6) with high bandwidth (~288 GB/s on the RX 7600 XT example) but CPU visibility depends on PCIe BAR mapping and platform configuration. GTT is system RAM mapped via the IOMMU; GPU access speed is limited by PCIe, but the CPU can access it quickly. amdgpu placement policy: 1) render targets, textures, and other GPU-intensive BOs prefer VRAM; 2) CPU-GPU shared command buffers (IBs) prefer GTT; 3) under memory pressure, the least-recently-used (LRU) VRAM BOs are evicted to GTT; 4) users can express a preference via the domain flags passed at GEM object creation time."
      },
      "completionChecklist": [
