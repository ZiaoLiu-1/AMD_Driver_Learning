        ...
        // ── Lesson 6.1.3 ──────────────────────────────────────
        {
          ...
          concept: {
            ...
            explanation: [
              ...
              'rocprof is a GPU performance analysis tool from the AMD ROCm ecosystem. It has three main modes: --stats mode (counts the execution time and number of calls of each GPU kernel), --hsa-trace mode (traces the complete timeline of API calls, memory copies, and kernel dispatch during HSA runtime), and hardware counter mode (specifies the GPU PMU counters to be collected through input.txt, such as SQ_WAVES, SQ_INSTS_VALU, TA_BUFFER_WAVEFRONTS_SUM). Note: rocprof requires the ROCm runtime, and the RX 7600 XT (gfx1102) is not on the official ROCm support list (check the official compatibility matrix) — on this card treat the lesson as conceptual, and use a supported device for real measurements.',
...
gfx1100.grbm.mmGRBM_STATUS == 0xC6008002
  ...
  SPI_BUSY          [23:22] = 1   ←Shaders working!`,
            hint: 'umr requires root privileges (GPU access via debugfs). If "cannot find ASIC" is reported, confirm that the amdgpu driver has been loaded. The ASIC name depends on your GPU model - the RX 7600 XT (Navi33) is gfx1102 (gfx1100 is Navi31 / the RX 7900 series).',
          },
          debugExercise: {
