...
export const cCppModuleEn: Module = {
  ...
  icon: 'Braces',
  description: 'Zero prior experience required: build fluency first with "C from Zero" and 32 warm-up drills, then systematically review the C needed for driver work, learn the C++ core relevant to the GPU software stack, and finish with a kernel-idioms practicum. The 26 micro-lessons pair with 72 Code Lab drills that compile and run in the browser with zero setup, bridging ordinary functions and arrays into real amdgpu / Mesa / ROCm / LLVM engineering contexts.',
  estimatedHours: 64,
  difficulty: 'beginner',
  subModules: [
    { id: 'cc-c0', title: 'C from Zero', titleEn: 'C from Zero' },
  ...
  theory: {
    overview: 'The later modules in this path read kernel source heavily, and reading code well requires a solid language foundation. This module is that "language bedrock": begin with 7 C-from-zero lessons that move from functions, expressions, branches and loops through arrays, pointers, strings, heap memory and mmap; continue with a 7-lesson systematic review of the C driver work truly needs (compilation & linking, types & integers, pointers, strings, structs & alignment, memory lifetime, function pointers & ops polymorphism); then take 6 lessons on the C++ core relevant to the GPU software stack (references & overloading, classes & RAII, copy/move, inheritance & virtual functions, templates, STL & smart pointers); and close with a 6-lesson kernel-idioms practicum (bit-op macros, intrusive lists, macro hygiene, the error-handling trio, kref & devm, concurrency contexts). Every lesson pairs with Code Lab practice (/code-lab): 72 problems — 32 C warm-ups (w-01~w-32, with the practice set and POSIX Bridge clearly optional) plus 40 kernel-flavored drills — compiled and run with real gcc/g++ in the browser, no local setup. Solving each matching drill after its lesson is strongly recommended. One mental thread runs throughout: kernel-side amdgpu is pure C, expressing polymorphism through hand-written ops structs and cleanup through goto staircases, while userspace Mesa, the ROCm/HIP runtime and LLVM use C++ virtual functions and RAII. After this module you can read both kernel C idioms and userspace C++ resource management and polymorphism, laying the language groundwork for every module that follows.',
    sections: [
      {
